import { type NotificationTaskPayload, type NotificationType } from './notifications-queue';
...
const normalizeName = (displayName: string): string =>
  displayName && displayName.trim().length > 0 ? displayName.trim() : 'there';

const renderInsightAlert = (
  payload: NotificationTaskPayload<'INSIGHT_ALERT'>
...
const renderStreakNudge = (
  payload: NotificationTaskPayload<'STREAK_NUDGE'>
): RenderedNotificationEmail => {
  const memberName = normalizeName(payload.recipient.displayName);
...
const renderModerationNotice = (
  payload: NotificationTaskPayload<'MODERATION_NOTICE'>
): RenderedNotificationEmail => {
  const memberName = normalizeName(payload.recipient.displayName);
...
const renderOnboardingWelcome = (
  payload: NotificationTaskPayload<'ONBOARDING_WELCOME'>
): RenderedNotificationEmail => {
  const memberName = normalizeName(payload.recipient.displayName);
...
const renderCommunityEvent = (
  payload: NotificationTaskPayload<'COMMUNITY_EVENT'>
): RenderedNotificationEmail => {
  const memberName = normalizeName(payload.recipient.displayName);
...
type NotificationBuilders = {
  [K in NotificationType]: (payload: NotificationTaskPayload<K>) => RenderedNotificationEmail;
};

const builders: NotificationBuilders = {
...
export const buildNotificationEmail = (payload: NotificationTaskPayload): RenderedNotificationEmail => {
  const builder = builders[payload.type] as (args: NotificationTaskPayload<typeof payload.type>) => RenderedNotificationEmail;
  const email = builder(payload as NotificationTaskPayload<typeof payload.type>);
