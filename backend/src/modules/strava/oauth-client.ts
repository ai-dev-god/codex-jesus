...
export type StravaAuthorizationExchange = {
  accessToken: string;
  refreshToken: string;
  expiresAt: Date;
  scope: string[];
...
const toAthleteProfile = (value: unknown): StravaAthleteProfile => {
...
export class StravaOAuthClient {
  constructor(
    private readonly clientId: string | null = env.STRAVA_CLIENT_ID ?? null,
    private readonly clientSecret: string | null = env.STRAVA_CLIENT_SECRET ?? null
  ) {}

  buildAuthorizeUrl(input: { state: string; redirectUri: string; scope: string[]; approvalPrompt?: 'auto' | 'force' }): string {
    if (!this.clientId) {
  ...
  async exchangeCode(params: { code: string; redirectUri: string }): Promise<StravaAuthorizationExchange> {
  ...
  async refreshToken(refreshToken: string): Promise<StravaAuthorizationExchange> {
    return this.requestToken({
  ...
  private async requestToken(params: TokenParams): Promise<StravaAuthorizationExchange> {
    ...
    if (!response.ok) {
      ...
      throw new StravaOAuthError(
        `Strava token request failed with status ${response.status}${detail ? `: ${detail.slice(0, 180)}` : ''}`,
        response.status
      );
    }
...
export const stravaOAuthClient = new StravaOAuthClient();
