...
import type { LongevityPlanJob, PrismaClient } from '@prisma/client';

import env from '../config/env';
import prismaClient from '../lib/prisma';
import { openRouterClient, type OpenRouterChatClient } from '../lib/openrouter';
import { baseLogger, type Logger } from '../observability/logger';
...
type MeasurementWithBiomarker = Prisma.BiomarkerMeasurementGetPayload<{
  include: { biomarker: true };
}>;

type LogWithBiomarker = Prisma.BiomarkerLogGetPayload<{
  include: { biomarker: true };
}>;
...
const parseJobPayload = (job: LongevityPlanJob): JobPayload => {
  const rawPayload = (job.payload ?? {}) as Record<string, unknown>;
  const request = (rawPayload.request ?? {}) as Record<string, unknown>;
  ...
  return {
    request: {
      ...
      riskTolerance:
        request.riskTolerance === 'low' || request.riskTolerance === 'high'
      ...
      lifestyleNotes:
        typeof request.lifestyleNotes === 'string' ? request.lifestyleNotes : null,
      retryOf: typeof request.retryOf === 'string' ? request.retryOf : null
    },
    metrics: typeof rawPayload.metrics === 'object' ? (rawPayload.metrics as Record<string, unknown>) : {}
  };
};

const buildPlannerPrompt = (context: PlannerContext): string => {
  return [
    'Context bundle (JSON):',
...
const parsePlanDraft = (raw: string): PlanDraft => {
  let parsed: unknown;
  try {
    parsed = JSON.parse(raw);
  } catch (error) {
    throw new Error(`Planner response was not valid JSON: ${(error as Error).message}`);
  }
  ...
  const focusAreas = Array.isArray(data.focusAreas)
    ...
    : [];
  const sections = Array.isArray(data.sections) ? (data.sections as PlanSection[]) : [];
  const evidence = Array.isArray(data.evidence) ? (data.evidence as PlanDraft['evidence']) : [];
...
const parseSafetyReview = (raw: string): SafetyReview => {
  let parsed: unknown;
  try {
    parsed = JSON.parse(raw);
  } catch (error) {
    throw new Error(`Safety response was not valid JSON: ${(error as Error).message}`);
  }
...
const parseRiskScorecard = (raw: string): RiskScoreEntry[] => {
  let parsed: unknown;
  try {
    parsed = JSON.parse(raw);
  } catch (error) {
    throw new Error(`Numeric response was not valid JSON: ${(error as Error).message}`);
  }
...
const toPlannerContext = (
  request: JobPayload['request'],
  measurements: MeasurementWithBiomarker[],
  logs: LogWithBiomarker[]
): PlannerContext => {
...
export const createLongevityPlanWorker = (deps: WorkerDeps = {}) => {
  const prisma = deps.prisma ?? prismaClient;
  const openRouter = deps.openRouter ?? openRouterClient;
  const logger = deps.logger ?? baseLogger.with({ component: 'longevity-plan-worker' });
  ...
  return async (taskName: string): Promise<void> => {
    ...
    const payloadWrapper = (metadata.payload ?? {}) as Record<string, unknown>;
    const inner = (payloadWrapper.payload ?? {}) as Record<string, unknown>;
    ...
    try {
      ...
      const plannerCompletion = await openRouter.createChatCompletion({
        model: env.OPENROUTER_PLANNER_MODEL,
        messages: [
      ...
      const numericCompletion = await openRouter.createChatCompletion({
        model: env.OPENROUTER_NUMERIC_MODEL,
        messages: [
      ...
      const safetyCompletion = await openRouter.createChatCompletion({
        model: env.OPENROUTER_SAFETY_MODEL,
        messages: [
      ...
      await prisma.longevityPlan.update({
        ...
        data: {
          ...
          focusAreas:
            planDraft.focusAreas.length > 0 ? planDraft.focusAreas : jobPayload.request.focusAreas,
