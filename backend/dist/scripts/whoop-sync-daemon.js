...
const decryptToken = (value) => {
    ...
    return token_crypto_1.whoopTokenCrypto.decrypt(value);
};
const refreshTokens = async (integration, refreshToken) => {
    if (!env_1.default.WHOOP_CLIENT_ID || !env_1.default.WHOOP_CLIENT_SECRET) {
        throw new Error('WHOOP_CLIENT_ID or WHOOP_CLIENT_SECRET is not configured');
    }
    const body = new URLSearchParams({
        grant_type: 'refresh_token',
        refresh_token: refreshToken,
        client_id: env_1.default.WHOOP_CLIENT_ID,
        client_secret: env_1.default.WHOOP_CLIENT_SECRET
    });
    ...
    const nextRefresh = typeof payload.refresh_token === 'string' && payload.refresh_token.length > 0
    ...
    const encryptedAccess = token_crypto_1.whoopTokenCrypto.encrypt(accessToken);
    const encryptedRefresh = token_crypto_1.whoopTokenCrypto.encrypt(nextRefresh);
