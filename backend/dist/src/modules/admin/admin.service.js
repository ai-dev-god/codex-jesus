...
const API_KEY_SUBSYSTEM_ERROR_MESSAGE = 'Service API keys are not available yet. Apply the latest database migrations and try again.';
const isTableMissingError = (error, tableName) => error instanceof client_1.Prisma.PrismaClientKnownRequestError &&
    error.code === 'P2021' &&
    (!tableName ||
        (typeof error.meta?.table === 'string' ? error.meta.table === tableName : true));
const isBackupSubsystemUnavailableError = (error) => isTableMissingError(error, 'AdminBackupJob');
const isApiKeySubsystemUnavailableError = (error) => isTableMissingError(error, 'ServiceApiKey');
...
const apiKeySubsystemUnavailableError = () => new http_error_1.HttpError(503, API_KEY_SUBSYSTEM_ERROR_MESSAGE, API_KEY_SUBSYSTEM_ERROR_CODE);
...
const LLM_ENGINE_CONFIG = {
    ...
    GEMINI_2_5_PRO: {
        ...
        costPer1K: env_1.default.OPENROUTER_GEMINI25_COST_PER_1K ?? 0.009,
        defaultTokens: 850
    }
};
const createEngineAccumulator = () => ({
    requests: 0,
    tokens: 0,
    costUsd: 0,
    latencies: []
...
class AdminService {
    ...
    async getLlmUsageMetrics(options = {}) {
        ...
        const engineAccumulators = new Map();
        ...
        for (const insight of insights) {
            ...
            for (const trace of traces) {
                ...
                const engineCost = typeof trace.costUsd === 'number' && Number.isFinite(trace.costUsd)
                    ? trace.costUsd
        ...
        const totalTokens = Math.round(Array.from(engineAccumulators.values()).reduce((sum, acc) => sum + acc.tokens, 0));
        ...
        const engines = ACTIVE_ENGINE_IDS.map((engineId) => {
            const accumulator = engineAccumulators.get(engineId) ?? createEngineAccumulator();
            const avgLatency = accumulator.latencies.length > 0
                ? Number((accumulator.latencies.reduce((sum, value) => sum + value, 0) / accumulator.latencies.length).toFixed(2))
                : null;
            return {
                id: engineId,
...
const startOfUtcDay = (input) => new Date(Date.UTC(input.getUTCFullYear(), input.getUTCMonth(), input.getUTCDate()));
...
const parseStoredUsage = (value) => {
    ...
    const record = value;
    const promptTokens = typeof record.promptTokens === 'number' && Number.isFinite(record.promptTokens) ? record.promptTokens : undefined;
...
const extractDualEngineTraces = (body) => {
...
const normalizeEngineIdentifier = (trace) => {
...
const normalizeEngineToken = (value) => {
    if (!value) {
        return null;
    }
...
const resolveTokensFromTrace = (engineId, usage) => {
...
const computeCostFromTokens = (engineId, tokens) => {
...
const getOrCreateAccumulator = (map, engineId) => {
...
const recordTimelineSample = (buckets, dateKey, engineId, requestsIncrement, costIncrement) => {
...
const roundCurrency = (value, precision = 4) => Number(value.toFixed(precision));
const getBucketValue = (entry, field) => {
    if (!entry) {
        return 0;
    }
    return field === 'requests' ? entry.requests : roundCurrency(entry.costUsd, 4);
};
const buildTimelineSeries = (windowDays, now, buckets, field) => {
    const series = [];
...
const computeFeatureUsageMetrics = (dualEngineInsightCount, audits) => {
